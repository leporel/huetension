...
// The palette dropdown drives the shared selection — picking a color
// here also moves the wheel highlight (and, in custom mode, the
// picker's edit target). `selectSlot` clamps the index for us.
const selected = computed<number>({
    get: () => workspace.selectedSlot,
    set: (i) => workspace.selectSlot(i),
});

const currentHex = computed(
    () => workspace.colors[selected.value]?.hex ?? "#000000",
);
...
const rows = computed<Row[]>(() => {
    const c = workspace.colors[selected.value];
    if (!c) return [];
...
<template>
    <div class="output">
        <div class="slot-row">
            <span class="swatch" :style="{ background: currentHex }" />
            <select
                v-model="selected"
                class="slot-select"
