...
onBeforeUnmount(() => {
  ...
  setGradedUrl(null);
  setGradedSpectrumUrl(null);
  if (originalSpectrumUrl.value) URL.revokeObjectURL(originalSpectrumUrl.value);
});
  ...
  <div class="export">
    ...
    <div v-if="isLUT" class="lut-panel lut-panel-split">
      <div class="lut-fields">
        ...
        <div class="lut-grid">
          <label class="lut-fld">
            <span>Radius <em>{{ lutRadius.toFixed(2) }}</em></span>
            <input
          ...
          <label class="lut-fld">
            <span>Distribution <em>{{ lutDistribution.toFixed(2) }}</em></span>
            <input
          ...
          <label class="lut-fld">
            <span>Intensity <em>{{ lutIntensity.toFixed(2) }}</em></span>
            <input
          ...
          <label class="lut-fld">
            <span>Blend <em>{{ lutBlend }} / {{ blendMax }}</em></span>
            <input
          ...
          <label class="lut-fld lut-fld-wide">
            ...
            <select v-model.number="lutSize" aria-label="LUT level">
              <option v-for="opt in LUT_LEVELS" :key="opt.level" :value="opt.value">
                {{ opt.label }}
              </option>
            </select>
          </label>
      ...
      <div class="hald-test">
        ...
        <div v-if="testImageUrl" class="hald-test-previews">
          <div class="hald-test-cell">
            ...
            <div v-if="originalSpectrumUrl" class="hald-test-spectrum">
              <img :src="originalSpectrumUrl" alt="reference spectrum" />
            </div>
          </div>
          <div class="hald-test-cell">
            <span class="hald-test-label">Graded</span>
            <div class="hald-test-frame">
              <img v-if="gradedUrl" :src="gradedUrl" alt="graded" />
            ...
            <div v-if="gradedSpectrumUrl" class="hald-test-spectrum">
              <img :src="gradedSpectrumUrl" alt="graded spectrum" />
            </div>
          </div>
        </div>
